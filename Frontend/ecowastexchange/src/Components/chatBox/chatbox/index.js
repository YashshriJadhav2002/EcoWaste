...
const Dashboard = () => {
	...
	const fetchMessages = async (conversationId, receiver) => {
		const res = await fetch(`/api/message/?conversationId=${conversationId}?senderId=${user}&&receiverId=${receiver?.receiverId}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			}
			...
			<div className=' w-[50%] h-screen bg-white flex flex-col items-center'>
				{
					...
					<div className='w-[75%] bg-secondary h-[80px] my-14 rounded-full flex items-center px-14 py-2'>
						<div className=' cursor-pointer'><img src={messages?.receiver?.Avatar} width={60} height={60} className="rounded-full" /></div>
